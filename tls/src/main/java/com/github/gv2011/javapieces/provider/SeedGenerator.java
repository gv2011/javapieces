...
abstract class SeedGenerator {
    ...
    static {
        final String egdSource = SunEntries.getSeedSource();
...
//        } else
     ...
     */
    static public void generateSeed(final byte[] result) {
    ...
    static byte[] getSystemEntropy() {
        ...
        final byte b =(byte)System.currentTimeMillis();
        md.update(b);
        ...
        java.security.AccessController.doPrivileged
            ((PrivilegedAction<Void>) () -> {
                try {
                    ...
                    // The temporary dir
                    final File f = new File(p.getProperty("java.io.tmpdir"));
                    int count = 0;
                    try (
     ...
     */
    private static void addNetworkAdapterInfo(final MessageDigest md) {

        try {
            final Enumeration<NetworkInterface> ifcs =
    ...
    */
    ...
    private static class ThreadedSeedGenerator extends SeedGenerator
        ...
        ThreadedSeedGenerator() {
            ...
            final Thread t = java.security.AccessController.doPrivileged
                ((PrivilegedAction<Thread>) () -> {
                    ...
                    final Thread newT = new Thread(finalsg[0],
            ...
            try {
                while (true) {
                    ...
                    for (counter = quanta = 0;
                        ...
                        try {
                            final BogusThread bt = new BogusThread();
                        ...
                        int latch = 0;
                        final long l = System.currentTimeMillis() + 250;
        ...
        @Override
        void getSeedBytes(final byte[] result) {
    ...
    static class URLSeedGenerator extends SeedGenerator {
        ...
        URLSeedGenerator(final String egdurl) throws IOException {
        if (egdurl == null) {
        ...
        void getSeedBytes(final byte[] result) {
            ...
            try {
                while (read < len) {
                    final int count = seedStream.read(result, read, len - read);
