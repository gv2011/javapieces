...
class InputRecord extends ByteArrayInputStream implements Record {
    ...
    void decrypt(final Authenticator authenticator,
            final CipherBox box) throws BadPaddingException {
        BadPaddingException reservedBPE = null;
        final int tagLen =
            (authenticator instanceof MAC) ? ((MAC)authenticator).MAClen() : 0;
        final int cipheredLength = count - headerSize;

        if (!box.isNullCipher()) {
            try {
                // apply explicit nonce for AEAD/CBC cipher suites if needed
                final int nonceSize = box.applyExplicitNonce(authenticator,
        ...
        if (authenticator instanceof MAC && tagLen != 0) {
            ...
            if (box.isCBCMode()) {
                final int remainingLen = calculateRemainingLen(
     ...
     */
    static boolean checkMacTags(final byte contentType, final byte[] buffer,
            final int offset, final int contentLen, final MAC signer, final boolean isSimulated) {

        final int tagLen = signer.MAClen();
        final byte[] hash = signer.compute(
    ...
    private static int[] compareMacTags(
            final byte[] buffer, final int offset, final byte[] tag) {
    ...
    static int calculateRemainingLen(
            final MAC signer, int fullLen, int usedLen) {
    ...
    private int readFully(final InputStream s, final byte b[], final int off, final int len)
            throws IOException {
        int n = 0;
        while (n < len) {
            final int readLen = s.read(b, off + n, len - n);
            if (readLen < 0) {
                return readLen;
            }
    ...
    void read(final InputStream s, final OutputStream o) throws IOException {
         ...
         */
        if(exlen < headerSize) {
            final int really = readFully(s, buf, exlen, headerSize - exlen);
            if (really < 0) {
     ...
     */
    static void checkRecordVersion(final ProtocolVersion version,
            final boolean allowSSL20Hello) throws SSLException {
     ...
     */
    private void readV3Record(final InputStream s, final OutputStream o)
            throws IOException {
        final ProtocolVersion recordVersion = ProtocolVersion.valueOf(buf[1], buf[2]);
         ...
         */
        final int contentLen = ((buf[3] & 0x0ff) << 8) + (buf[4] & 0xff);
        ...
        if (contentLen > buf.length - headerSize) {
            final byte[] newbuf = new byte[contentLen + headerSize];
            System.arraycopy(buf, 0, newbuf, 0, headerSize);
            buf = newbuf;
        }

        if (exlen < contentLen + headerSize) {
            final int really = readFully(
     ...
     */
    private void handleUnknownRecord(final InputStream s, final OutputStream o)
            throws IOException {
        /*
        ...
        if (((buf[0] & 0x080) != 0) && buf[2] == 1) {
            ...
            final ProtocolVersion recordVersion =
     ...
     * action.
     */
    void writeBuffer(final OutputStream s, final byte [] buf, final int off, final int len)
            throws IOException {
        s.write(buf, 0, len);
        s.flush();
    }
