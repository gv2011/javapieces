...
final class SunEntries {
    ...
    static void putEntries(final Map<Object, Object> map) {
...
//        final boolean nativeAvailable = false;
//        final boolean useNativePRNG = seedSource.equals(URL_DEV_URANDOM) ||
...
//        }
        ...
        final String dsaKeyClasses = "java.security.interfaces.DSAPublicKey" +
     ...
     * signed providers.
     */
    static File getDeviceFile(final URL device) throws IOException {
        try {
            final URI deviceURI = device.toURI();
            if(deviceURI.isOpaque()) {
                // File constructor does not accept opaque URI
                final URI localDir = new File(
                    System.getProperty("user.dir")).toURI();
                final String uriPath = localDir.toString() +
