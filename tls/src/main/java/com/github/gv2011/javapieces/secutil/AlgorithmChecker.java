...
import com.github.gv2011.javapieces.provider.PKIX;
...
public final class AlgorithmChecker extends PKIXCertPathChecker {
     ...
     */
    public AlgorithmChecker(final TrustAnchor anchor, final String variant) {
     ...
     *                passed will set it to Validator.GENERIC.
     */
    public AlgorithmChecker(final AlgorithmConstraints constraints,
            final Timestamp jarTimestamp, final String variant) {
     ...
     *                passed will set it to Validator.GENERIC.
     */
    public AlgorithmChecker(final TrustAnchor anchor,
            final AlgorithmConstraints constraints, final Date pkixdate,
            final Timestamp jarTimestamp, final String variant) {

        if (anchor != null) {
            if (anchor.getTrustedCert() != null) {
                trustedPubKey = anchor.getTrustedCert().getPublicKey();
     ...
     *                passed will set it to Validator.GENERIC.
     */
    public AlgorithmChecker(final TrustAnchor anchor, final Date pkixdate, final String variant) {
        this(anchor, certPathDefaultConstraints, pkixdate, null, variant);
    }

    // Check this 'cert' for restrictions in the AnchorCertificates
    // trusted certificates list
    private static boolean checkFingerprint(final X509Certificate cert) {
        if (!publicCALimits) {
            return false;
        }
    ...
    public void check(final Certificate cert,
        ...
        final boolean[] keyUsage = ((X509Certificate) cert).getKeyUsage();
        ...
        final AlgorithmParameters currSigAlgParams = algorithmId.getParameters();
        PublicKey currPubKey = cert.getPublicKey();
        final String currSigAlg = x509Cert.getSigAlgName();
        ...
        final ConstraintsParameters cp =
        ...
        if (PKIX.isDSAPublicKeyWithoutParams(currPubKey)) {
            ...
            final DSAParams params = ((DSAPublicKey)prevPubKey).getParams();
            if (params == null) {
                throw new CertPathValidatorException(
            ...
            try {
                final BigInteger y = ((DSAPublicKey)currPubKey).getY();
                final KeyFactory kf = KeyFactory.getInstance("DSA");
                final DSAPublicKeySpec ks = new DSAPublicKeySpec(y, params.getP(),
     ...
     *     certificate
     */
    void trySetTrustAnchor(final TrustAnchor anchor) {
     ...
     *                passed will set it to Validator.GENERIC.
     */
    public static void check(final PublicKey key, final X509CRL crl, final String variant)
                        throws CertPathValidatorException {
        ...
        final AlgorithmId algorithmId = x509CRLImpl.getSigAlgId();
     ...
     *                passed will set it to Validator.GENERIC.
     */
    static void check(final PublicKey key, final AlgorithmId algorithmId, final String variant)
                        throws CertPathValidatorException {
        final String sigAlgName = algorithmId.getName();
        final AlgorithmParameters sigAlgParams = algorithmId.getParameters();
