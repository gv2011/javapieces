...
public final class PKCS12KeyStore extends KeyStoreSpi {
    ...
    private static class CertEntry extends Entry {
        ...
        CertEntry(final X509Certificate cert, final byte[] keyId, final String alias) {
            this(cert, keyId, alias, null, null);
        }

        CertEntry(final X509Certificate cert, final byte[] keyId, final String alias,
                final ObjectIdentifier[] trustedKeyUsage,
                final Set<? extends KeyStore.Entry.Attribute> attributes) {
            date = new Date();
            this.cert = cert;
            this.keyId = keyId;
            this.alias = alias;
        ...
        throws NoSuchAlgorithmException, UnrecoverableKeyException
    {
        ...
        try {
            ...
            final EncryptedPrivateKeyInfo encrInfo =
                        new EncryptedPrivateKeyInfo(encrBytes);
            encryptedKey = encrInfo.getEncryptedData();

            // parse Algorithm parameters
            final DerValue val = new DerValue(encrInfo.getAlgorithm().encode());
            final DerInputStream in = val.toDerInputStream();
            algOid = in.getOID();
            algParams = parseAlgParameters(algOid, in);

        } catch (final IOException ioe) {
            final UnrecoverableKeyException uke =
        ...
        try {
            ...
            while (true) {
                try {
                    // Use JCE
                    final SecretKey skey = getPBEKey(password);
             ...
             */
            final DerValue val = new DerValue(keyInfo);
            final DerInputStream in = val.toDerInputStream();
            @SuppressWarnings("unused")
            final int i = in.getInteger();
            final DerValue[] value = in.getSequence(2);
            final AlgorithmId algId = new AlgorithmId(value[0].getOID());
            final String keyAlgo = algId.getName();

            // decode private key
            if (entry instanceof PrivateKeyEntry) {
                final KeyFactory kfac = KeyFactory.getInstance(keyAlgo);
                final PKCS8EncodedKeySpec kspec = new PKCS8EncodedKeySpec(keyInfo);
     ...
     */
    @Override
    public synchronized void engineSetKeyEntry(final String alias, final Key key,
        ...
        throws KeyStoreException
    {
        ...
        try {
            ...
            } catch (final DestroyFailedException dfe) {
                // ignore
            }
        }
    }
     ...
     */
    private void setKeyEntry(final String alias, final Key key,
        final KeyStore.PasswordProtection passwordProtection, final Certificate[] chain,
        final Set<KeyStore.Entry.Attribute> attributes)
            throws KeyStoreException
    {
        try {
            Entry entry;
            ...
            if (key instanceof PrivateKey) {
                final PrivateKeyEntry keyEntry = new PrivateKeyEntry();
                ...
                final SecretKeyEntry keyEntry = new SecretKeyEntry();
                keyEntry.date = new Date();

                // Encode secret key in a PKCS#8
                @SuppressWarnings("resource")
                final DerOutputStream pkcs8 = new DerOutputStream();
                final DerOutputStream secretKeyInfo = new DerOutputStream();
                secretKeyInfo.putInteger(0);
                final AlgorithmId algId = AlgorithmId.get(key.getAlgorithm());
     ...
     */
    @Override
    public synchronized void engineSetKeyEntry(final String alias, final byte[] key,
        ...
        throws KeyStoreException
    {
        ...
        final PrivateKeyEntry entry = new PrivateKeyEntry();
        entry.date = new Date();
     ...
     */
    private AlgorithmParameters getAlgorithmParameters(final String algorithm)
        throws IOException
    {
     ...
     * parse Algorithm Parameters
     */
    private AlgorithmParameters parseAlgParameters(final ObjectIdentifier algorithm,
     ...
     * Generate PBE key
     */
    private SecretKey getPBEKey(final char[] password) throws IOException
    {
        SecretKey skey = null;

        try {
            final PBEKeySpec keySpec = new PBEKeySpec(password);
            final SecretKeyFactory skFac = SecretKeyFactory.getInstance("PBE");
     ...
     */
    private byte[] encryptPrivateKey(final byte[] data,
        final KeyStore.PasswordProtection passwordProtection)
        throws IOException, NoSuchAlgorithmException, UnrecoverableKeyException
    {
        ...
        try {
            ...
            if (algorithm != null) {
                final AlgorithmParameterSpec algParamSpec =
            ...
            final ObjectIdentifier pbeOID = mapPBEAlgorithmToOID(algorithm);
            if (pbeOID == null) {
            ...
            // Use JCE
            final SecretKey skey = getPBEKey(passwordProtection.getPassword());
            final Cipher cipher = Cipher.getInstance(algorithm);
            cipher.init(Cipher.ENCRYPT_MODE, skey, algParams);
            final byte[] encryptedKey = cipher.doFinal(data);
     ...
     */
    private static ObjectIdentifier mapPBEAlgorithmToOID(final String algorithm)
     ...
     */
    private static String mapPBEParamsToAlgorithm(final ObjectIdentifier algorithm,
        final AlgorithmParameters algParams) throws NoSuchAlgorithmException {
        // Check for PBES2 algorithms
     ...
     */
    @Override
    public synchronized void engineSetCertificateEntry(final String alias,
     ...
     */
    private void setCertEntry(final String alias, final Certificate cert,
        final Set<KeyStore.Entry.Attribute> attributes) throws KeyStoreException {
     ...
     */
    @Override
    public synchronized void engineDeleteEntry(final String alias)
        throws KeyStoreException
    {
        if (debug != null) {
        ...
        final Entry entry = entries.get(alias.toLowerCase(Locale.ENGLISH));
        if (entry instanceof PrivateKeyEntry) {
            final PrivateKeyEntry keyEntry = (PrivateKeyEntry) entry;
    ...
    @Override
    public boolean
        engineEntryInstanceOf(final String alias,
    ...
    public String engineGetCertificateAlias(final Certificate cert) {
        ...
        for (final Enumeration<String> e = engineAliases(); e.hasMoreElements(); ) {
            ...
            final Entry entry = entries.get(alias);
            if (entry instanceof PrivateKeyEntry) {
                if (((PrivateKeyEntry) entry).chain != null) {
     ...
     */
    @Override
    public synchronized void engineStore(final OutputStream stream, final char[] password)
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        ...
        // -- Create PFX
        final DerOutputStream pfx = new DerOutputStream();

        // PFX version (always write the latest version)
        @SuppressWarnings("resource")
        final DerOutputStream version = new DerOutputStream();
        version.putInteger(VERSION_3);
        final byte[] pfxVersion = version.toByteArray();
        pfx.write(pfxVersion);

        // -- Create AuthSafe
        final DerOutputStream authSafe = new DerOutputStream();

        // -- Create ContentInfos
        final DerOutputStream authSafeContentInfo = new DerOutputStream();
        ...
        if (privateKeyCount > 0 || secretKeyCount > 0) {
            ...
            final byte[] safeContentData = createSafeContent();
            final ContentInfo dataContentInfo = new ContentInfo(safeContentData);
        ...
        if (certificateCount > 0) {
            ...
            final byte[] encrData = createEncryptedData(password);
            final ContentInfo encrContentInfo =
        ...
        @SuppressWarnings("resource")
        final DerOutputStream cInfo = new DerOutputStream();
        cInfo.write(DerValue.tag_SequenceOf, authSafeContentInfo);
        final byte[] authenticatedSafe = cInfo.toByteArray();

        // Create Encapsulated ContentInfo
        final ContentInfo contentInfo = new ContentInfo(authenticatedSafe);
        contentInfo.encode(authSafe);
        final byte[] authSafeData = authSafe.toByteArray();
        pfx.write(authSafeData);

        // -- MAC
        final byte[] macData = calculateMac(password, authenticatedSafe);
        pfx.write(macData);

        // write PFX to output stream
        @SuppressWarnings("resource")
        final DerOutputStream pfxout = new DerOutputStream();
        pfxout.write(DerValue.tag_Sequence, pfx);
        final byte[] pfxData = pfxout.toByteArray();
    ...
    public KeyStore.Entry engineGetEntry(final String alias,
        ...
        if (protParam instanceof KeyStore.PasswordProtection) {
            if (engineIsCertificateEntry(alias)) {
                ...
                final KeyStore.PasswordProtection pp =
                        (KeyStore.PasswordProtection)protParam;
                final char[] password = pp.getPassword();

                final Key key = engineGetKey(alias, password);
                if (key instanceof PrivateKey) {
                    final Certificate[] chain = engineGetCertificateChain(alias);
     ...
     */
    @Override
    public synchronized void engineSetEntry(final String alias, final KeyStore.Entry entry,
        ...
        if (entry instanceof KeyStore.TrustedCertificateEntry) {
            ...
            if (pProtect == null || pProtect.getPassword() == null) {
                ...
                final KeyStore.SecretKeyEntry ske = (KeyStore.SecretKeyEntry)entry;
    ...
    private Set<KeyStore.Entry.Attribute> getAttributes(final Entry entry) {
        ...
        // localKeyID
        final byte[] keyIdValue = entry.keyId;
        if (keyIdValue != null) {
            entry.attributes.add(new PKCS12Attribute(
        ...
        // trustedKeyUsage
        if (entry instanceof CertEntry) {
            final ObjectIdentifier[] trustedKeyUsageValue =
                ((CertEntry) entry).trustedKeyUsage;
     ...
     */
    @SuppressWarnings("unused")
    private byte[] generateHash(final byte[] data) throws IOException
    {
        byte[] digest = null;
     ...
     */
    private byte[] calculateMac(final char[] passwd, final byte[] data)
        throws IOException
    {
        byte[] mData = null;
        final String algName = "SHA1";
        ...
        try {
            ...
            final Mac m = Mac.getInstance("HmacPBESHA1");
            final PBEParameterSpec params =
                        new PBEParameterSpec(salt, iterationCount);
            final SecretKey key = getPBEKey(passwd);
            m.init(key, params);
            m.update(data);
            final byte[] macResult = m.doFinal();

            // encode as MacData
            final MacData macData = new MacData(algName, macResult, salt,
                                                iterationCount);
            @SuppressWarnings("resource")
     ...
     */
    private boolean validateChain(final Certificate[] certChain)
    {
        ...
        // the chain
        final Set<Certificate> set = new HashSet<>(Arrays.asList(certChain));
     ...
     */
    private byte[] getBagAttributes(final String alias, final byte[] keyId,
        final Set<KeyStore.Entry.Attribute> attributes) throws IOException {
        return getBagAttributes(alias, keyId, null, attributes);
    }

    private byte[] getBagAttributes(final String alias, final byte[] keyId,
        final ObjectIdentifier[] trustedUsage,
        final Set<KeyStore.Entry.Attribute> attributes) throws IOException {
        ...
        // SafeBag Attributes
        @SuppressWarnings("resource")
        final DerOutputStream bagAttrs = new DerOutputStream();

        // Encode the friendlyname oid.
        if (alias != null) {
            final DerOutputStream bagAttr1 = new DerOutputStream();
            bagAttr1.putOID(PKCS9FriendlyName_OID);
            final DerOutputStream bagAttrContent1 = new DerOutputStream();
            @SuppressWarnings("resource")
            final DerOutputStream bagAttrValue1 = new DerOutputStream();
        ...
        if (keyId != null) {
            final DerOutputStream bagAttr2 = new DerOutputStream();
            bagAttr2.putOID(PKCS9LocalKeyId_OID);
            final DerOutputStream bagAttrContent2 = new DerOutputStream();
            @SuppressWarnings("resource")
            final DerOutputStream bagAttrValue2 = new DerOutputStream();
        ...
        if (trustedUsage != null) {
            final DerOutputStream bagAttr3 = new DerOutputStream();
            bagAttr3.putOID(TrustedKeyUsage_OID);
            final DerOutputStream bagAttrContent3 = new DerOutputStream();
            @SuppressWarnings("resource")
            final DerOutputStream bagAttrValue3 = new DerOutputStream();
            for (final ObjectIdentifier usage : trustedUsage) {
        ...
        final DerOutputStream attrs = new DerOutputStream();
        ...
        if (attributes != null) {
            for (final KeyStore.Entry.Attribute attribute : attributes) {
     ...
     */
    private byte[] createEncryptedData(final char[] password)
        throws CertificateException, IOException
    {
        ...
        for (final Enumeration<String> e = engineAliases(); e.hasMoreElements(); ) {
            ...
            for (int i = 0; i < certs.length; i++) {
                ...
                final DerOutputStream safeBag = new DerOutputStream();
                safeBag.putOID(CertBag_OID);

                // create a CertBag
                final DerOutputStream certBag = new DerOutputStream();
                certBag.putOID(PKCS9CertType_OID);

                // write encoded certs in a context-specific tag
                final DerOutputStream certValue = new DerOutputStream();
                ...
                @SuppressWarnings("resource")
                final DerOutputStream certout = new DerOutputStream();
                certout.write(DerValue.tag_Sequence, certBag);
                final byte[] certBagValue = certout.toByteArray();

                // Wrap the CertBag encoding in a context-specific tag.
                final DerOutputStream bagValue = new DerOutputStream();
        ...
        @SuppressWarnings("resource")
        final DerOutputStream safeBagValue = new DerOutputStream();
        safeBagValue.write(DerValue.tag_SequenceOf, out);
        final byte[] safeBagData = safeBagValue.toByteArray();

        // encrypt the content (EncryptedContentInfo)
        final byte[] encrContentInfo = encryptContent(safeBagData, password);

        // -- SEQUENCE of EncryptedData
        final DerOutputStream encrData = new DerOutputStream();
        @SuppressWarnings("resource")
        final DerOutputStream encrDataContent = new DerOutputStream();
    ...
    private byte[] createSafeContent()
        ...
        for (final Enumeration<String> e = engineAliases(); e.hasMoreElements(); ) {
            ...
            if (keyEntry instanceof PrivateKeyEntry) {
                ...
                // get the encrypted private key
                final byte[] encrBytes = ((PrivateKeyEntry)keyEntry).protectedPrivKey;
            ...
            // DER encode the secret key
                ...
                // Create a SecretBag
                final DerOutputStream secretBag = new DerOutputStream();
                secretBag.putOID(PKCS8ShroudedKeyBag_OID);

                // Write secret key in a context-specific tag
                final DerOutputStream secretKeyValue = new DerOutputStream();
                ...
                final DerOutputStream secretBagSeq = new DerOutputStream();
                secretBagSeq.write(DerValue.tag_Sequence, secretBag);
                final byte[] secretBagValue = secretBagSeq.toByteArray();
     ...
     */
    private byte[] encryptContent(final byte[] data, final char[] password)
        throws IOException {

        byte[] encryptedData = null;

        // create AlgorithmParameters
        final AlgorithmParameters algParams =
                getAlgorithmParameters("PBEWithSHA1AndRC2_40");
        final DerOutputStream bytes = new DerOutputStream();
        final AlgorithmId algId =
                new AlgorithmId(pbeWithSHAAnd40BitRC2CBC_OID, algParams);
        algId.encode(bytes);
        final byte[] encodedAlgId = bytes.toByteArray();

        try {
            // Use JCE
            final SecretKey skey = getPBEKey(password);
            final Cipher cipher = Cipher.getInstance("PBEWithSHA1AndRC2_40");
            cipher.init(Cipher.ENCRYPT_MODE, skey, algParams);
        ...
        // create EncryptedContentInfo
        final DerOutputStream bytes2 = new DerOutputStream();
        bytes2.putOID(ContentInfo.DATA_OID);
        bytes2.write(encodedAlgId);

        // Wrap encrypted data in a context-specific tag.
        final DerOutputStream tmpout2 = new DerOutputStream();
     ...
     */
    @Override
    public synchronized void engineLoad(final InputStream stream, char[] password)
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        @SuppressWarnings("unused")
        final DataInputStream dis;
        @SuppressWarnings("unused")
        final CertificateFactory cf = null;
        @SuppressWarnings("unused")
        ...
        final DerValue val = new DerValue(stream);
        final DerInputStream s = val.toDerInputStream();
        final int version = s.getInteger();
         ...
         */
        byte[] authSafeData;
        final ContentInfo authSafe = new ContentInfo(s);
        ...
        final DerInputStream as = new DerInputStream(authSafeData);
        final DerValue[] safeContentsArray = as.getSequence(2);
        final int count = safeContentsArray.length;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (contentType.equals((Object)ContentInfo.DATA_OID)) {
                ...
                final DerInputStream edi =
                                safeContents.getContent().toDerInputStream();
                @SuppressWarnings("unused")
                final int edVersion = edi.getInteger();
                final DerValue[] seq = edi.getSequence(2);
                @SuppressWarnings("unused")
                final ObjectIdentifier edContentType = seq[0].getOID();
                ...
                // parse Algorithm parameters
                final DerInputStream in = seq[1].toDerInputStream();
                final ObjectIdentifier algOid = in.getOID();
                final AlgorithmParameters algParams = parseAlgParameters(algOid, in);

                while (true) {
                    try {
                        // Use JCE
                        final SecretKey skey = getPBEKey(password);
                        final Cipher cipher = Cipher.getInstance(algOid.toString());
                        cipher.init(Cipher.DECRYPT_MODE, skey, algParams);
            ...
            final DerInputStream sc = new DerInputStream(safeContentsData);
            loadSafeContents(sc, password);
        }

        // The MacData is optional.
        if (password != null && s.available() > 0) {
           final MacData macData = new MacData(s);
           try {
                String algName =
                ...
                final Mac m = Mac.getInstance("HmacPBE" + algName);
                final PBEParameterSpec params =
                        new PBEParameterSpec(macData.getSalt(),
                                        macData.getIterations());
                final SecretKey key = getPBEKey(password);
                m.init(key, params);
         ...
         */
        final PrivateKeyEntry[] list =
            keyList.toArray(new PrivateKeyEntry[keyList.size()]);
        for (int m = 0; m < list.length; m++) {
            final PrivateKeyEntry entry = list[m];
            if (entry.keyId != null) {
                ...
                while (cert != null) {
                    ...
                    if (!chain.isEmpty()) {
                        for (final X509Certificate chainCert : chain) {
     ...
     */
    private X509Certificate findMatchedCertificate(final PrivateKeyEntry entry) {
        CertEntry keyIdMatch = null;
        CertEntry aliasMatch = null;
        for (final CertEntry ce: certEntries) {
    ...
    private void loadSafeContents(final DerInputStream stream, final char[] password)
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        final DerValue[] safeBags = stream.getSequence(2);
        final int count = safeBags.length;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (bagId.equals((Object)PKCS8ShroudedKeyBag_OID)) {
                final PrivateKeyEntry kEntry = new PrivateKeyEntry();
                kEntry.protectedPrivKey = bagValue.toByteArray();
                bagItem = kEntry;
                privateKeyCount++;
            } else if (bagId.equals((Object)CertBag_OID)) {
                final DerInputStream cs = new DerInputStream(bagValue.toByteArray());
                final DerValue[] certValues = cs.getSequence(2);
                @SuppressWarnings("unused")
                final ObjectIdentifier certId = certValues[0].getOID();
                if (!certValues[1].isContextSpecific((byte)0)) {
                    throw new IOException("unsupported PKCS12 cert value type "
                                        + certValues[1].tag);
                }
                final DerValue certValue = certValues[1].data.getDerValue();
                ...
                final DerInputStream ss = new DerInputStream(bagValue.toByteArray());
                final DerValue[] secretValues = ss.getSequence(2);
                @SuppressWarnings("unused")
                final ObjectIdentifier secretId = secretValues[0].getOID();
                if (!secretValues[1].isContextSpecific((byte)0)) {
                    throw new IOException(
                        "unsupported PKCS12 secret value type "
                                        + secretValues[1].tag);
                }
                final DerValue secretValue = secretValues[1].data.getDerValue();
                final SecretKeyEntry kEntry = new SecretKeyEntry();
            ...
            final Set<PKCS12Attribute> attributes = new HashSet<>();

            if (attrSet != null) {
                for (int j = 0; j < attrSet.length; j++) {
                    final byte[] encoded = attrSet[j].toByteArray();
                    final DerInputStream as = new DerInputStream(encoded);
                    final DerValue[] attrSeq = as.getSequence(2);
                    final ObjectIdentifier attrId = attrSeq[0].getOID();
                    final DerInputStream vs =
            ...
            if (bagItem instanceof KeyEntry) {
                final KeyEntry entry = (KeyEntry)bagItem;
                ...
                final String keyIdStr = new String(keyId, "UTF8");
                Date date = null;
                ...
                final X509Certificate cert = (X509Certificate)bagItem;
