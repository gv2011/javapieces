...
public final class KeyUtil {
    ...
    public static final int getKeySize(final Key key) {
        ...
        if (key instanceof SecretKey) {
            final SecretKey sk = (SecretKey)key;
            final String format = sk.getFormat();
            if ("RAW".equals(format) && sk.getEncoded() != null) {
                size = (sk.getEncoded().length * 8);
            }   // Otherwise, it may be a unextractable key of PKCS#11, or
                // a key we are not able to handle.
        } else if (key instanceof RSAKey) {
            final RSAKey pubk = (RSAKey)key;
            size = pubk.getModulus().bitLength();
        } else if (key instanceof ECKey) {
            final ECKey pubk = (ECKey)key;
            size = pubk.getParams().getOrder().bitLength();
        } else if (key instanceof DSAKey) {
            final DSAKey pubk = (DSAKey)key;
            final DSAParams params = pubk.getParams();    // params can be null
            size = (params != null) ? params.getP().bitLength() : -1;
        } else if (key instanceof DHKey) {
            final DHKey pubk = (DHKey)key;
     ...
     */
    public static final void validate(final Key key)
     ...
     */
    public static final void validate(final KeySpec keySpec)
            throws InvalidKeyException {
        if (keySpec == null) {
            throw new NullPointerException(
     ...
     */
    public static final boolean isOracleJCEProvider(final String providerName) {
    ...
    public static byte[] checkTlsPreMasterSecretKey(
            final int clientVersion, final int serverVersion, SecureRandom random,
            byte[] encoded, final boolean isFailOver) {
     ...
     */
    private static void validateDHPublicKey(final DHPublicKey publicKey)
            throws InvalidKeyException {
        final DHParameterSpec paramSpec = publicKey.getParams();

        final BigInteger p = paramSpec.getP();
        final BigInteger g = paramSpec.getG();
        final BigInteger y = publicKey.getY();

        validateDHPublicKey(p, g, y);
    }

    private static void validateDHPublicKey(final DHPublicKeySpec publicKeySpec)
            throws InvalidKeyException {
        validateDHPublicKey(publicKeySpec.getP(),
            publicKeySpec.getG(), publicKeySpec.getY());
    }

    private static void validateDHPublicKey(final BigInteger p,
            final BigInteger g, final BigInteger y) throws InvalidKeyException {

        // For better interoperability, the interval is limited to [2, p-2].
        final BigInteger leftOpen = BigInteger.ONE;
        final BigInteger rightOpen = p.subtract(BigInteger.ONE);
        ...
        final BigInteger r = p.remainder(y);
