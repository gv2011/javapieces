...
final class EngineInputRecord extends InputRecord {
    ...
    int bytesInCompletePacket(final ByteBuffer buf) throws SSLException {
        ...
        if (formatVerified ||
             ...
             */
            final boolean isShort = ((byteZero & 0x80) != 0);
            ...
            if (isShort &&
                 ...
                 */
                final int mask = (isShort ? 0x7f : 0x3f);
     ...
     * process.
     */
    ByteBuffer decrypt(final Authenticator authenticator,
            final CipherBox box, final ByteBuffer bb) throws BadPaddingException {
     ...
     */
    private static boolean checkMacTags(final byte contentType, final ByteBuffer bb,
            final MAC signer, final boolean isSimulated) {
    ...
    void writeBuffer(final OutputStream s, final byte [] buf, final int off, final int len)
         ...
         */
        final ByteBuffer netBB = (ByteBuffer)
    ...
    ByteBuffer read(final ByteBuffer srcBB) throws IOException {
        ...
        final ProtocolVersion recordVersion = ProtocolVersion.valueOf(
         ...
         */
        final int len = bytesInCompletePacket(srcBB);
        assert(len > 0);

        if (debug != null && Debug.isOn("packet")) {
            try {
                final HexDumpEncoder hd = new HexDumpEncoder();
                final ByteBuffer bb = srcBB.duplicate();  // Use copy of BB
                bb.limit(srcPos + len);
        ...
        // operate on.
        final ByteBuffer bb = srcBB.slice();
