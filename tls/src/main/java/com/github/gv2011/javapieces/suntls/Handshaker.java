...
import com.github.gv2011.javapieces.suntls.CipherSuite.MacAlg;
...
abstract class Handshaker {
    ...
    Handshaker(final SSLSocketImpl c, final SSLContextImpl context,
            final ProtocolList enabledProtocols, final boolean needCertVerify,
            final boolean isClient, final ProtocolVersion activeProtocolVersion,
    ...
    Handshaker(final SSLEngineImpl engine, final SSLContextImpl context,
            final ProtocolList enabledProtocols, final boolean needCertVerify,
            final boolean isClient, final ProtocolVersion activeProtocolVersion,
            final boolean isInitialHandshake, final boolean secureRenegotiation,
            final byte[] clientVerifyData, final byte[] serverVerifyData) {
    ...
    private void init(final SSLContextImpl context, final ProtocolList enabledProtocols,
            final boolean needCertVerify, final boolean isClient,
    ...
    void fatalSE(final byte b, final String diagnostic, final Throwable cause)
            throws IOException {
        if (conn != null) {
    ...
    void setIdentificationProtocol(final String protocol) {
        identificationProtocol = protocol;
    }
     ...
     * Does not check if the required server certificates are available.
     */
    final static boolean isNegotiable(final CipherSuiteList proposed, final CipherSuite s) {
    ...
    CipherSuiteList getActiveCipherSuites() {
        if (activeCipherSuites == null) {
            ...
            final ArrayList<CipherSuite> suites = new ArrayList<>();
            if (!(activeProtocols.collection().isEmpty()) &&
                    activeProtocols.min.v != ProtocolVersion.NONE.v) {
                boolean checkedCurves = false;
                boolean hasCurves = false;
                for (final CipherSuite suite : enabledCipherSuites.collection()) {
    ...
    ProtocolList getActiveProtocols() {
        if (activeProtocols == null) {
            ...
            boolean checkedCurves = false;
            boolean hasCurves = false;
            final ArrayList<ProtocolVersion> protocols = new ArrayList<>(4);
            for (final ProtocolVersion protocol : enabledProtocols.collection()) {
    ...
    CipherBox newReadCipher() throws NoSuchAlgorithmException {
        final BulkCipher cipher = cipherSuite.cipher;
        CipherBox box;
        if (isClient) {
    ...
    Authenticator newReadAuthenticator()
        ...
        if (cipherSuite.cipher.cipherType == AEAD_CIPHER) {
            ...
            final MacAlg macAlg = cipherSuite.macAlg;
            if (isClient) {
     ...
     */
    void process_record(final InputRecord r, final boolean expectingFinished)
            throws IOException {
    ...
    void kickstart() throws IOException {
        ...
        final HandshakeMessage m = getKickstartMessage();
     ...
     */
    void sendChangeCipherSpec(final Finished mesg, final boolean lastMessage)
            throws IOException {
    ...
    void calculateKeys(final SecretKey preMasterSecret, final ProtocolVersion version) {
        final SecretKey master = calculateMasterSecret(preMasterSecret, version);
     ...
     */
    private SecretKey calculateMasterSecret(final SecretKey preMasterSecret,
        ...
        final TlsMasterSecretParameterSpec spec = new TlsMasterSecretParameterSpec(
    ...
    void calculateConnectionKeys(final SecretKey masterKey) {
        ...
        final TlsKeyMaterialParameterSpec spec = new TlsKeyMaterialParameterSpec(
     ...
     */
    static void throwSSLException(final String msg, final Throwable cause)
            throws SSLException {
