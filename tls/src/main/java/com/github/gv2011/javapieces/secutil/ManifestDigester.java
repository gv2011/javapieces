...
public class ManifestDigester {
    ...
    private boolean findSection(final int offset, final Position pos)
    {
        int i = offset;
    ...
    public ManifestDigester(final byte bytes[])
    {
        rawBytes = bytes;
        entries = new HashMap<>();
        ...
        while(findSection(start, pos)) {
            final int len = pos.endOfFirstLine-start+1;
            final int sectionLen = pos.endOfSection-start+1;
            final int sectionLenWithBlank = pos.startOfNext-start;

            if (len > 6) {
                if (isNameAttr(bytes, start)) {
                    final StringBuilder nameBuf = new StringBuilder(sectionLen);
    ...
    private boolean isNameAttr(final byte bytes[], final int start)
    {
    ...
    public static class Entry {
        ...
        public Entry(final int offset, final int length,
                     final int lengthWithBlankLine, final byte[] rawBytes)
        {
            this.offset = offset;
            this.length = length;
        ...
        public byte[] digest(final MessageDigest md)
        {
            md.reset();
            if (oldStyle) {
        ...
        private void doOldStyle(final MessageDigest md,
                                final byte[] bytes,
                                final int offset,
                                final int length)
        {
        ...
        public byte[] digestWorkaround(final MessageDigest md)
        {
            md.reset();
    ...
    public Entry get(final String name, final boolean oldStyle) {
    ...
    public byte[] manifestDigest(final MessageDigest md)
        {
            md.reset();
