...
public class JulianCalendar extends BaseCalendar {
    ...
    private static class Date extends BaseCalendar.Date {
        ...
        public String toString() {
            ...
            if (era != null) {
                final String n = era.getAbbreviation();
                if (n != null) {
                    sb.append(n).append(' ');
                }
            }
     ...
     */
    @Override
    public long getFixedDate(final int jyear, final int month, final int dayOfMonth, final BaseCalendar.Date cache) {
    ...
    public void getCalendarDateFromFixedDate(final CalendarDate date, final long fixedDate) {
        final Date jdate = (Date) date;
        final long fd = 4 * (fixedDate - JULIAN_EPOCH) + 1464;
        int year;
        if (fd >= 0) {
            year = (int)(fd / 1461);
        } else {
        ...
        final boolean isLeap = CalendarUtils.isJulianLeapYear(year);
        ...
        final int dayOfMonth = (int)(fixedDate - getFixedDate(year, month, 1, jdate)) + 1;
    ...
    public int getYearFromFixedDate(final long fixedDate) {
        final int year = (int) CalendarUtils.floorDivide(4 * (fixedDate - JULIAN_EPOCH) + 1464, 1461);
        return year;
    }
    ...
    @Override
    boolean isLeapYear(final int jyear) {
