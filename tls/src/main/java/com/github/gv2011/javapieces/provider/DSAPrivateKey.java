...
import com.github.gv2011.javapieces.x509.AlgIdDSA;
...
implements java.security.interfaces.DSAPrivateKey, Serializable {
     ...
     */
    public DSAPrivateKey(final BigInteger x, final BigInteger p,
                         final BigInteger q, final BigInteger g)
    throws InvalidKeyException {
        ...
        try {
            ...
            final InvalidKeyException ike = new InvalidKeyException(
     ...
     */
    public DSAPrivateKey(final byte[] encoded) throws InvalidKeyException {
        clearOldKey();
        decode(encoded);
    }
    ...
    public DSAParams getParams() {
        try {
            if (algid instanceof DSAParams) {
                ...
                final AlgorithmParameters algParams = algid.getParameters();
                if (algParams == null) {
                    return null;
                }
    ...
    protected void parseKeyBits() throws InvalidKeyException {
        try {
            final DerInputStream in = new DerInputStream(key);
            x = in.getBigInteger();
        } catch (final IOException e) {
            final InvalidKeyException ike = new InvalidKeyException(e.getMessage());
            ike.initCause(e);
            throw ike;
        }
    }
