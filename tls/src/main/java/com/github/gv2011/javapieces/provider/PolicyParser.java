...
public class PolicyParser {
    ...
    private String expand(final String value, final boolean encodeURL)
        throws PropertyExpander.ExpandException
    {
        if (!expandProp) {
            return value;
        ...
        throws ParsingException, IOException
    {
        ...
        while (lookahead != StreamTokenizer.TT_EOF) {
            if (peek("grant")) {
                ...
                final DomainEntry de = parseDomainEntry();
                if (de != null) {
                    final String domainName = de.getName();
                    if (!domainEntries.containsKey(domainName)) {
                        ...
                        final Object[] source = {domainName};
    ...
    public void add(final GrantEntry ge)
    {
        grantEntries.addElement(ge);
    }

    public void replace(final GrantEntry origGe, final GrantEntry newGe)
    {
        grantEntries.setElementAt(newGe, grantEntries.indexOf(origGe));
    }

    public boolean remove(final GrantEntry ge)
    {
    ...
    public String getKeyStoreUrl() {
        ...
        } catch (final PropertyExpander.ExpandException peee) {
            if (debug != null) {
    ...
    public void setKeyStoreUrl(final String url) {
    ...
    public void setKeyStoreProvider(final String provider) {
    ...
    public void write(final Writer policy)
    {
        final PrintWriter out = new PrintWriter(new BufferedWriter(policy));

        final Enumeration<GrantEntry> enum_ = grantElements();
        ...
        while (enum_.hasMoreElements()) {
            final GrantEntry ge = enum_.nextElement();
            ge.write(out);
            out.println();
        }
        out.flush();
     ...
     */
    private void writeKeyStoreEntry(final PrintWriter out) {
        out.print("keystore \"");
    ...
    private void writeStorePassURL(final PrintWriter out) {
    ...
    private GrantEntry parseGrantEntry()
        throws ParsingException, IOException
    {
        final GrantEntry e = new GrantEntry();
        ...
        while(!peek("{")) {
            ...
            if (peekAndMatch("Codebase")) {
                ...
                final StringTokenizer aliases = new StringTokenizer(e.signedBy,
                                                              ",", true);
                int actr = 0;
                int cctr = 0;
                while (aliases.hasMoreTokens()) {
                    final String alias = aliases.nextToken().trim();
                    if (alias.equals(","))
                        cctr++;
        ...
        while(!peek("}")) {
            if (peek("Permission")) {
                try {
                    final PermissionEntry pe = parsePermissionEntry();
                    e.add(pe);
                } catch (final PropertyExpander.ExpandException peee) {
        ...
        try {
            ...
            if (e.codeBase != null) {
                ...
                if ((es=e.codeBase.indexOf(EXTDIRS_EXPANSION)) < 0) {
                    ...
                    final String[] extDirs = parseExtDirs(e.codeBase, es);
                    if (extDirs != null && extDirs.length > 0) {
                        for (int i = 0; i < extDirs.length; i++) {
                            final GrantEntry newGe = (GrantEntry)e.clone();
    ...
    {
        final PermissionEntry e = new PermissionEntry();
    ...
    private DomainEntry parseDomainEntry()
        throws ParsingException, IOException
    {
        final boolean ignoreEntry = false;
     ...
     */
    private Map<String, String> parseProperties(final String terminator)
        throws ParsingException, IOException {
    ...
    // package-private: used by PolicyFile for static policy
    static String[] parseExtDirs(final String codebase, final int start) {

        final String s = System.getProperty(EXTDIRS_PROPERTY);
        final String globalPrefix = (start > 0 ? codebase.substring(0, start) : "file:");
        final int end = start + EXTDIRS_EXPANSION.length();
        final String globalSuffix = (end < codebase.length() ? codebase.substring(end) :
            (String) null);
        ...
        if (s != null) {
            ...
            for (int i = 0; i < count; i++) {
                final File file = new File(st.nextToken());
    ...
    private boolean peekAndMatch(final String expect)
        throws ParsingException, IOException
    {
        if (peek(expect)) {
            match(expect);
    ...
    private boolean peek(final String expect) {
        boolean found = false;
    ...
    private String match(final String expect)
        throws ParsingException, IOException
    {
        String value = null;
    ...
    public static class GrantEntry {
        ...
        public void add(final PermissionEntry pe)
        {
            permissionEntries.addElement(pe);
        }

        public boolean remove(final PrincipalEntry pe)
        {
            return principals.remove(pe);
        }

        public boolean remove(final PermissionEntry pe)
        {
            return permissionEntries.removeElement(pe);
        }

        public boolean contains(final PrincipalEntry pe)
        {
            return principals.contains(pe);
        }

        public boolean contains(final PermissionEntry pe)
        {
        ...
        public void write(final PrintWriter out) {
            ...
            if (principals != null && principals.size() > 0) {
                final Iterator<PrincipalEntry> pli = principals.iterator();
                while (pli.hasNext()) {
                    out.print("      ");
                    final PrincipalEntry pe = pli.next();
                    pe.write(out);
                    if (pli.hasNext())
                        out.print(",\n");
                }
            ...
            out.println(" {");
            final Enumeration<PermissionEntry> enum_ = permissionEntries.elements();
            while (enum_.hasMoreElements()) {
                final PermissionEntry pe = enum_.nextElement();
                out.write("  ");
                pe.write(out);
            }
            out.println("};");
        ...
        public Object clone() {
            ...
            ge.codeBase = codeBase;
            ge.signedBy = signedBy;
            ge.principals = new LinkedList<>(principals);
            ge.permissionEntries =
                        new Vector<>(permissionEntries);
            return ge;
        }
    }
    ...
    public static class PrincipalEntry implements Principal {
        ...
        public String getDisplayName(final boolean addQuote) {
            if (isWildcardName()) {
                return "*";
            }
            else {
        ...
        public boolean equals(final Object obj) {
            ...
            final PrincipalEntry that = (PrincipalEntry)obj;
    ...
    public static class PermissionEntry {
        ...
        public PermissionEntry(final String permission,
                        final String name,
                        final String action) {
        ...
        public boolean equals(final Object obj) {
            ...
            final PermissionEntry that = (PermissionEntry) obj;
            ...
            if (signedBy == null) {
                ...
                if (!signedBy.equals(that.signedBy)) return false;
            }
    ...
    static class DomainEntry {
        ...
        void add(final KeyStoreEntry entry) throws ParsingException {
            final String keystoreName = entry.getName();
            if (!entries.containsKey(keystoreName)) {
                ...
                final Object[] source = {keystoreName};
                throw new ParsingException(form.format(source));
            }
        }
        ...
        public String toString() {
            ...
            if (entries != null) {
                for (final KeyStoreEntry entry : entries.values()) {
    ...
    public static class ParsingException extends GeneralSecurityException {
        ...
        public ParsingException(final int line, final String msg) {
            super("line " + line + ": " + msg);
            final MessageFormat form = new MessageFormat
                (ResourcesMgr.getString("line.number.msg"));
            final Object[] source = {new Integer(line), msg};
            i18nMessage = form.format(source);
        }

        public ParsingException(final int line, final String expect, final String actual) {
            super("line " + line + ": expected [" + expect +
                "], found [" + actual + "]");
            final MessageFormat form = new MessageFormat(ResourcesMgr.getString
                ("line.number.expected.expect.found.actual."));
            final Object[] source = {new Integer(line), expect, actual};
            i18nMessage = form.format(source);
        }
    ...
    public static void main(final String arg[]) throws Exception {
        try (FileReader fr = new FileReader(arg[0]);
            ...
            final PolicyParser pp = new PolicyParser(true);
            pp.read(fr);
            pp.write(fw);
        }
    }
