...
public class CertificatePolicySet {
     ...
     */
    public CertificatePolicySet(final DerInputStream in) throws IOException {
        ids = new Vector<>();
        final DerValue[] seq = in.getSequence(5);

        for (int i = 0; i < seq.length; i++) {
            final CertificatePolicyId id = new CertificatePolicyId(seq[i]);
            ids.addElement(id);
        }
    }
    ...
    public String toString() {
        final String s = "CertificatePolicySet:[\n"
                 + ids.toString()
                 + "]\n";
