...
import com.github.gv2011.javapieces.pkcs.PKCS7;
import com.github.gv2011.javapieces.pkcs.SignerInfo;
...
public class SignatureFileVerifier {
     ...
     */
    public SignatureFileVerifier(final ArrayList<CodeSigner[]> signerCache,
     ...
     *
     */
    public boolean needSignatureFile(final String name)
    {
    ...
    public static boolean isSigningRelated(String name) {
        ...
        if (isBlockOrSF(name) || name.equals("MANIFEST.MF")) {
            ...
            if (extIndex != -1) {
                final String ext = name.substring(extIndex + 1);
                ...
                for (int index = 0; index < ext.length(); index++) {
                    final char cc = ext.charAt(index);
     ...
     *
     */
    public void process(final Hashtable<String, CodeSigner[]> signers,
            final List<Object> manifestDigests)
    ...
    private void processImpl(final Hashtable<String, CodeSigner[]> signers,
            final List<Object> manifestDigests)
        throws IOException, SignatureException, NoSuchAlgorithmException,
            JarException, CertificateException
    {
        ...
        final SignerInfo[] infos = block.verify(sfBytes);
        ...
        final CodeSigner[] newSigners = getSigners(infos, block);
         ...
         */
        for (final CodeSigner s: newSigners) {
            if (debug != null) {
        ...
        final Iterator<Map.Entry<String,Attributes>> entries =
                                sf.getEntries().entrySet().iterator();

        // see if we can verify the whole manifest first
        final boolean manifestSigned = verifyManifestHash(sf, md, manifestDigests);
        ...
        while(entries.hasNext()) {
            ...
            final Map.Entry<String,Attributes> e = entries.next();
     ...
     */
    boolean permittedCheck(final String key, final String algorithm) {
        final Boolean permitted = permittedAlgs.get(algorithm);
        if (permitted == null) {
            try {
     ...
     */
    String getWeakAlgorithms(final String header) {
        String w = "";
        try {
            for (final String key : permittedAlgs.keySet()) {
     ...
     */
    private boolean verifyManifestHash(final Manifest sf,
                                       final ManifestDigester md,
                                       final List<Object> manifestDigests)
         throws IOException, SignatureException
    {
        ...
        for (final Map.Entry<Object,Object> se : mattr.entrySet()) {

            final String key = se.getKey().toString();

            if (key.toUpperCase(Locale.ENGLISH).endsWith("-DIGEST-MANIFEST")) {
                // 16 is length of "-Digest-Manifest"
                final String algorithm = key.substring(0, key.length()-16);
                validEntry = true;
                ...
                if (digest != null) {
                    final byte[] computedHash = md.manifestDigest(digest);
    ...
    private boolean verifyManifestMainAttrs(final Manifest sf, final ManifestDigester md)
         throws IOException, SignatureException
    {
        final Attributes mattr = sf.getMainAttributes();
        ...
        for (final Map.Entry<Object,Object> se : mattr.entrySet()) {
            ...
            if (key.toUpperCase(Locale.ENGLISH).endsWith(ATTR_DIGEST)) {
                ...
                final MessageDigest digest = getDigest(algorithm);
                if (digest != null) {
                    final ManifestDigester.Entry mde =
                        md.get(ManifestDigester.MF_MAIN_ATTRS, false);
                    final byte[] computedHash = mde.digest(digest);
                    final byte[] expectedHash =
                        Base64.getMimeDecoder().decode((String)se.getValue());
    ...
    private boolean verifySection(final Attributes sfAttr,
                                  final String name,
                                  final ManifestDigester md)
         throws IOException, SignatureException
    {
        boolean oneDigestVerified = false;
        final ManifestDigester.Entry mde = md.get(name,block.isOldStyle());
        // If only weak algorithms are used.
        boolean weakAlgs = true;
        ...
        if (sfAttr != null) {
            ...
            for (final Map.Entry<Object,Object> se : sfAttr.entrySet()) {
                final String key = se.getKey().toString();

                if (key.toUpperCase(Locale.ENGLISH).endsWith("-DIGEST")) {
                    // 7 is length of "-Digest"
                    final String algorithm = key.substring(0, key.length()-7);
                    validEntry = true;
     ...
     */
    private CodeSigner[] getSigners(final SignerInfo[] infos, final PKCS7 block)
        ...
        for (int i = 0; i < infos.length; i++) {
            ...
            final SignerInfo info = infos[i];
            final ArrayList<X509Certificate> chain = info.getCertificateChain(block);
            final CertPath certChain = certificateFactory.generateCertPath(chain);
            if (signers == null) {
    ...
    static boolean contains(final CodeSigner[] set, final CodeSigner signer)
    {
    ...
    static boolean isSubSet(final CodeSigner[] subset, final CodeSigner[] set)
    {
     ...
     */
    static boolean matches(final CodeSigner[] signers, final CodeSigner[] oldSigners,
        final CodeSigner[] newSigners) {
    ...
    void updateSigners(final CodeSigner[] newSigners,
        final Hashtable<String, CodeSigner[]> signers, final String name) {

        final CodeSigner[] oldSigners = signers.get(name);
