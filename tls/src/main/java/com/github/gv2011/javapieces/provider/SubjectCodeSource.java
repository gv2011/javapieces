...
import com.github.gv2011.javapieces.provider.PolicyParser.PrincipalEntry;
...
class SubjectCodeSource extends CodeSource implements java.io.Serializable {
    ...
    private static final java.util.ResourceBundle rb =
        ...
        ((PrivilegedAction<ResourceBundle>) () -> (java.util.ResourceBundle.getBundle
    ...
    private static final Debug debug =
        Debug.getInstance("auth", "\t[Auth Access]");
     ...
     *                  <code>SubjectCodeSource</code> <p>
     */
    SubjectCodeSource(final Subject subject,
        final LinkedList<PrincipalEntry> principals,
        final URL url, final Certificate[] certs) {
    ...
    public boolean implies(final CodeSource codesource) {
        ...
        final SubjectCodeSource that = (SubjectCodeSource)codesource;
        ...
        final ListIterator<PrincipalEntry> li = principals.listIterator(0);
        while (li.hasNext()) {
            final PrincipalEntry pppe = li.next();
            try {

                // use new Principal.implies method

                final Class<?> pClass = Class.forName(pppe.principalClass,
                                                true, sysClassLoader);
                ...
                final Constructor<?> c = pClass.getConstructor(PARAMS);
    ...
    private boolean subjectListImpliesPrincipalEntry(
                final LinkedList<PrincipalEntry> subjectList, final PrincipalEntry pppe) {

        final ListIterator<PrincipalEntry> li = subjectList.listIterator(0);
        while (li.hasNext()) {
            final PrincipalEntry listPppe = li.next();
    ...
    public boolean equals(final Object obj) {
        ...
        final SubjectCodeSource that = (SubjectCodeSource)obj;

        // the principal lists must match
        try {
            if (getSubject() != that.getSubject())
                return false;
        } catch (final SecurityException se) {
            return false;
        }

        if ((principals == null && that.principals != null) ||
            (principals != null && that.principals == null))
            return false;

        if (principals != null && that.principals != null) {
            if (!principals.containsAll(that.principals) ||
                !that.principals.containsAll(principals))
    ...
    public String toString() {
        ...
        if (getSubject() != null) {
            if (debug != null) {
                ...
                returnMe = returnMe + "\n" +
                        java.security.AccessController.doPrivileged
                                ((PrivilegedAction<String>) () -> finalSubject.toString());
            } else {
                returnMe = returnMe + "\n" + getSubject().toString();
            }
        }
        if (principals != null) {
            final ListIterator<PrincipalEntry> li = principals.listIterator();
            while (li.hasNext()) {
                final PrincipalEntry pppe = li.next();
