...
import static com.github.gv2011.javapieces.suntls.CipherSuite.KeyExchange.K_RSA;
import static com.github.gv2011.javapieces.suntls.CipherSuite.KeyExchange.K_RSA_EXPORT;
...
final class ClientHandshaker extends Handshaker {
     ...
     * Constructors
     */
    ClientHandshaker(final SSLSocketImpl socket, final SSLContextImpl context,
            final ProtocolList enabledProtocols,
    ...
    ClientHandshaker(final SSLEngineImpl engine, final SSLContextImpl context,
            final ProtocolList enabledProtocols,
            final ProtocolVersion activeProtocolVersion,
            final boolean isInitialHandshake, final boolean secureRenegotiation,
            final byte[] clientVerifyData, final byte[] serverVerifyData) {
    ...
    void processMessage(final byte type, final int messageLen) throws IOException {
        ...
        case HandshakeMessage.ht_hello_request:
            serverHelloRequest(new HelloRequest(input));
            break;

        case HandshakeMessage.ht_server_hello:
            serverHello(new ServerHello(input, messageLen));
            break;
        ...
        case HandshakeMessage.ht_certificate:
            ...
            serverCertificate(new CertificateMsg(input));
            serverKey =
        ...
        case HandshakeMessage.ht_certificate_request:
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                ...
                final Collection<SignatureAndHashAlgorithm> supportedPeerSignAlgs =
        ...
        case HandshakeMessage.ht_server_hello_done:
            serverHelloDone(new ServerHelloDone(input));
            break;
     ...
     */
    private void serverHelloRequest(final HelloRequest mesg) throws IOException {
        if (debug != null && Debug.isOn("handshake")) {
     ...
     */
    private void serverHello(final ServerHello mesg) throws IOException {
        ...
        final RenegotiationInfoExtension serverHelloRI = (RenegotiationInfoExtension)
        ...
        if (session != null) {
            ...
            if (session.getSessionId().equals(mesg.sessionId)) {
                ...
                final CipherSuite sessionSuite = session.getSuite();
                ...
                final ProtocolVersion sessionVersion = session.getProtocolVersion();
                ...
                if (sessionSuite.keyExchange == K_KRB5 ||
                    ...
                    final Principal localPrincipal = session.getLocalPrincipal();

                    Subject subject = null;
                    try {
                        subject = AccessController.doPrivileged(
                            (PrivilegedExceptionAction<Subject>) () -> Krb5Helper.getClientSubject(getAccSE()));
     ...
     */
    private void serverKeyExchange(final RSA_ServerKeyExchange mesg)
     ...
     */
    private void serverKeyExchange(final DH_ServerKeyExchange mesg)
            throws IOException {
        if (debug != null && Debug.isOn("handshake")) {
            mesg.print(System.out);
        }
    ...
    private void serverKeyExchange(final ECDH_ServerKeyExchange mesg)
            throws IOException {
        if (debug != null && Debug.isOn("handshake")) {
            mesg.print(System.out);
        }
     ...
     */
    private void serverHelloDone(final ServerHelloDone mesg) throws IOException {
        if (debug != null && Debug.isOn("handshake")) {
            mesg.print(System.out);
        }
        /*
        ...
        if (certRequest != null) {
            ...
            final ArrayList<String> keytypesTmp = new ArrayList<>(4);
        ...
        case K_ECDH_ECDSA:
            ...
            final ECParameterSpec params = ((ECPublicKey)serverKey).getParams();
        ...
        case K_KRB5_EXPORT:
            ...
            for (final SNIServerName serverName : requestedServerNames) {
            ...
            if (kerberosMsg == null) {
                final String hostname = getHostSE();
                if (hostname == null) {
     ...
     */
    private void serverFinished(final Finished mesg) throws IOException {
        if (debug != null && Debug.isOn("handshake")) {
            mesg.print(System.out);
        }

        final boolean verified = mesg.verify(handshakeHash, Finished.SERVER,
    ...
    HandshakeMessage getKickstartMessage() throws SSLException {
        ...
        if (session != null) {
            ...
            if (!allowUnsafeServerCertChange && session.isSessionResumption()) {
                ...
                } catch (final SSLPeerUnverifiedException puve) {
        ...
        if (session != null) {
            ...
            if (!enableNewSession) {
                ...
                final Collection<CipherSuite> cipherList = new ArrayList<>(2);
        ...
        if (secureRenegotiation && cipherSuites.contains(CipherSuite.C_SCSV)) {
            final Collection<CipherSuite> cipherList =
        ...
        final ClientHello clientHelloMessage = new ClientHello(
                sslContext.getSecureRandom(), maxProtocolVersion,
                sessionId, cipherSuites);

        // add elliptic curves and point format extensions
        if (cipherSuites.containsEC()) {
            final SupportedEllipticCurvesExtension ece =
        ...
        if (maxProtocolVersion.v >= ProtocolVersion.TLS12.v) {
            ...
            final Collection<SignatureAndHashAlgorithm> localSignAlgs =
                                                getLocalSupportedSignAlgs();
     ...
     */
    private void serverCertificate(final CertificateMsg mesg) throws IOException {
        if (debug != null && Debug.isOn("handshake")) {
            mesg.print(System.out);
        }
        ...
        if (reservedServerCerts != null) {
            ...
            final String identityAlg = getEndpointIdentificationAlgorithmSE();
     ...
     */
    private static boolean isIdentityEquivalent(final X509Certificate thisCert,
        ...
        if ((thisSubjectAltNames != null) && (prevSubjectAltNames != null)) {
            ...
            final Collection<String> thisSubAltIPAddrs =
                        getSubjectAltNames(thisSubjectAltNames, ALTNAME_IP);
            final Collection<String> prevSubAltIPAddrs =
            ...
            final Collection<String> thisSubAltDnsNames =
                        getSubjectAltNames(thisSubjectAltNames, ALTNAME_DNS);
            final Collection<String> prevSubAltDnsNames =
    ...
    private static Collection<String> getSubjectAltNames(
            final Collection<List<?>> subjectAltNames, final int type) {

        HashSet<String> subAltDnsNames = null;
        for (final List<?> subjectAltName : subjectAltNames) {
            final int subjectAltNameType = (Integer)subjectAltName.get(0);
    ...
    private static boolean isEquivalent(final Collection<String> thisSubAltNames,
            final Collection<String> prevSubAltNames) {
