...
import com.github.gv2011.javapieces.provider.UntrustedChecker;
import com.github.gv2011.javapieces.x509.NetscapeCertTypeExtension;
...
public final class SimpleValidator extends Validator {
    ...
    X509Certificate[] engineValidate(X509Certificate[] chain,
            final Collection<X509Certificate> otherCerts,
            final AlgorithmConstraints constraints,
            final Object parameter) throws CertificateException {
        ...
        final AlgorithmChecker defaultAlgChecker =
    ...
    private int checkExtensions(final X509Certificate cert, final int maxPathLen)
    ...
    private void checkNetscapeCertType(final X509Certificate cert,
            final Set<String> critSet) throws CertificateException {
    ...
    static boolean getNetscapeCertTypeBit(final X509Certificate cert, final String type) {
        try {
            ...
            if (cert instanceof X509CertImpl) {
                ...
                final ObjectIdentifier oid = OBJID_NETSCAPE_CERT_TYPE;
                ext = (NetscapeCertTypeExtension)certImpl.getExtension(oid);
                if (ext == null) {
                    return true;
                }
            } else {
                final byte[] extVal = cert.getExtensionValue(OID_NETSCAPE_CERT_TYPE);
                if (extVal == null) {
                    return true;
                }
    ...
    private int checkBasicConstraints(final X509Certificate cert,
            final Set<String> critSet, int maxPathLen) throws CertificateException {
     ...
     * certificates.
     */
    private void checkKeyUsage(final X509Certificate cert, final Set<String> critSet)
            throws CertificateException {
     ...
     */
    private X509Certificate[] buildTrustedChain(final X509Certificate[] chain)
            throws CertificateException {
        final List<X509Certificate> c = new ArrayList<>(chain.length);
        // scan chain starting at EE cert
        // if a trusted certificate is found, append it and return
        for (int i = 0; i < chain.length; i++) {
            final X509Certificate cert = chain[i];
            final X509Certificate trustedCert = getTrustedCertificate(cert);
            if (trustedCert != null) {
                c.add(trustedCert);
        ...
        final X509Certificate cert = chain[chain.length - 1];
        @SuppressWarnings("unused")
        final X500Principal subject = cert.getSubjectX500Principal();
        final X500Principal issuer = cert.getIssuerX500Principal();
        final List<X509Certificate> list = trustedX500Principals.get(issuer);
        if (list != null) {
            final X509Certificate trustedCert = list.iterator().next();
            c.add(trustedCert);
            return c.toArray(CHAIN0);
        }
    ...
    private X509Certificate getTrustedCertificate(final X509Certificate cert) {
        ...
        final List<X509Certificate> list = trustedX500Principals.get(certSubjectName);
        if (list == null) {
            return null;
        }
