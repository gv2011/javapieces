...
public abstract class CertStoreHelper {
        ...
        throws NoSuchAlgorithmException
    {
        ...
        try {
            helper = AccessController.doPrivileged(
                (PrivilegedExceptionAction<CertStoreHelper>) () -> {
                    try {
                        final Class<?> c = Class.forName(cl, true, null);
                        final CertStoreHelper csh
    ...
    static boolean isCausedByNetworkIssue(final String type, final CertStoreException cse) {
        switch (type) {
            case "LDAP":
            case "SSLServer":
                try {
                    final CertStoreHelper csh = CertStoreHelper.getInstance(type);
                    return csh.isCausedByNetworkIssue(cse);
                } catch (final NoSuchAlgorithmException nsae) {
                    return false;
                }
            case "URI":
                final Throwable t = cse.getCause();
