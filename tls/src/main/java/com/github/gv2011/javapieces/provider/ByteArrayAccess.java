...
final class ByteArrayAccess {
    ...
    static {
        final boolean scaleOK = ((unsafe.arrayIndexScale(byte[].class) == 1)
    ...
    private static boolean unaligned() {
        final String arch = java.security.AccessController.doPrivileged
    ...
    static void b2lBig(final byte[] in, int inOfs, final long[] out, int outOfs, int len) {
        ...
        if (littleEndianUnaligned) {
            ...
            len += inOfs;
            while (inOfs < len) {
                final int i1 = ((in[inOfs + 3] & 0xff)      )
                       | ((in[inOfs + 2] & 0xff) <<  8)
                       | ((in[inOfs + 1] & 0xff) << 16)
                       | ((in[inOfs    ]       ) << 24);
                inOfs += 4;
                final int i2 = ((in[inOfs + 3] & 0xff)      )
                       | ((in[inOfs + 2] & 0xff) <<  8)
                       | ((in[inOfs + 1] & 0xff) << 16)
                       | ((in[inOfs    ]       ) << 24);
