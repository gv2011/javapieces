...
import static com.github.gv2011.javapieces.provider.ByteArrayAccess.*;
...
abstract class SHA2 extends DigestBase {
     ...
     */
    SHA2(final String name, final int digestLength, final int[] initialHashes) {
    ...
    void implDigest(final byte[] out, final int ofs) {
        ...
        final int index = (int)bytesProcessed & 0x3f;
     ...
     * @param z int
     */
    private static int lf_ch(final int x, final int y, final int z) {
        return (x & y) ^ ((~x) & z);
    }
     ...
     * @param z int
     */
    private static int lf_maj(final int x, final int y, final int z) {
        return (x & y) ^ (x & z) ^ (y & z);
    }
     ...
     * @param s int
     */
    private static int lf_R( final int x, final int s ) {
        return (x >>> s);
    }
     ...
     * @param s int
     */
    private static int lf_S(final int x, final int s) {
        return (x >>> s) | (x << (32 - s));
    }
     ...
     * @param x int
     */
    private static int lf_sigma0(final int x) {
     ...
     * @param x int
     */
    private static int lf_sigma1(final int x) {
     ...
     * @param x int
     */
    private static int lf_delta0(final int x) {
     ...
     * @param x int
     */
    private static int lf_delta1(final int x) {
     ...
     */
    @Override
    void implCompress(final byte[] buf, final int ofs) {
        b2iBig64(buf, ofs, W);
        ...
        for (int i = 0; i < ITERATION; i++) {
            final int T1 = h + lf_sigma1(e) + lf_ch(e,f,g) + ROUND_CONSTS[i] + W[i];
            final int T2 = lf_sigma0(a) + lf_maj(a,b,c);
            h = g;
            g = f;
            f = e;
            e = d + T1;
    ...
    public Object clone() throws CloneNotSupportedException {
        final SHA2 copy = (SHA2) super.clone();
