...
//          this.valueN = 256;
     ...
     */
    @Override
    protected void engineInit(final AlgorithmParameterSpec genParamSpec,
                              final SecureRandom random)
        throws InvalidAlgorithmParameterException {
        if (!(genParamSpec instanceof DSAGenParameterSpec)) {
            throw new InvalidAlgorithmParameterException("Invalid parameter");
        }
        final DSAGenParameterSpec dsaGenParams = (DSAGenParameterSpec) genParamSpec;
        final int primePLen = dsaGenParams.getPrimePLength();
        if (primePLen > 2048) {
            throw new InvalidParameterException
        ...
        valueL = primePLen;
        valueN = dsaGenParams.getSubprimeQLength();
        seedLen = dsaGenParams.getSeedLength();
        this.random = random;
    }
    ...
    protected AlgorithmParameters engineGenerateParameters() {
        ...
        try {
            ...
            if (valueL == -1) {
                try {
                    engineInit(DEFAULTS, random);
                } catch (final InvalidAlgorithmParameterException iape) {
                    // should never happen
                }
            }
            final BigInteger[] pAndQ = generatePandQ(random, valueL,
                                               valueN, seedLen);
            final BigInteger paramP = pAndQ[0];
            final BigInteger paramQ = pAndQ[1];
            final BigInteger paramG = generateG(paramP, paramQ);

            final DSAParameterSpec dsaParamSpec =
     ...
     * at index 3.
     */
    private static BigInteger[] generatePandQ(final SecureRandom random, final int valueL,
                                              final int valueN, final int seedLen) {
        String hashAlg = null;
        if (valueN == 160) {
            hashAlg = "SHA";
        } else if (valueN == 224) {
        ...
        final int outLen = hashObj.getDigestLength()*8;
        final int n = (valueL - 1) / outLen;
        final int b = (valueL - 1) % outLen;
        final byte[] seedBytes = new byte[seedLen/8];
        final BigInteger twoSl = TWO.pow(seedLen);
        ...
        while (true) {
            do {
                ...
                /* Step 6 */
                final BigInteger U = new BigInteger(1, hashObj.digest(seedBytes)).
            ...
            for (counter = 0; counter < 4*valueL; counter++) {
                final BigInteger V[] = new BigInteger[n + 1];
                /* Step 11.1 */
                for (int j = 0; j <= n; j++) {
                    final BigInteger J = BigInteger.valueOf(j);
                    final BigInteger tmp = (seed.add(offset).add(J)).mod(twoSl);
                    final byte[] vjBytes = hashObj.digest(toByteArray(tmp));
                ...
                /* Step 11.3 */
                final BigInteger twoLm1 = TWO.pow(valueL - 1);
                final BigInteger X = W.add(twoLm1);
                /* Step 11.4, 11.5 */
                final BigInteger c = X.mod(resultQ.multiply(TWO));
                resultP = X.subtract(c.subtract(ONE));
                /* Step 11.6, 11.7 */
                if (resultP.compareTo(twoLm1) > -1
                    && resultP.isProbablePrime(primeCertainty)) {
                    /* Step 11.8 */
                    final BigInteger[] result = {resultP, resultQ, seed,
     ...
     */
    private static BigInteger generateG(final BigInteger p, final BigInteger q) {
        BigInteger h = ONE;
        /* Step 1 */
        final BigInteger pMinusOneOverQ = (p.subtract(ONE)).divide(q);
     ...
     */
    private static byte[] toByteArray(final BigInteger bigInt) {
        byte[] result = bigInt.toByteArray();
        if (result[0] == 0) {
            final byte[] tmp = new byte[result.length - 1];
