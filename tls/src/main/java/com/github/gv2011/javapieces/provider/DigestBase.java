...
abstract class DigestBase extends MessageDigestSpi implements Cloneable {
    ...
    protected final void engineUpdate(final byte[] b, int ofs, int len) {
        ...
        if (bufOfs != 0) {
            final int n = Math.min(len, blockSize - bufOfs);
    ...
    // compress complete blocks
    private int implCompressMultiBlock(final byte[] b, int ofs, final int limit) {
    ...
    protected final byte[] engineDigest() {
        final byte[] b = new byte[digestLength];
        try {
    ...
    @Override
    protected final int engineDigest(final byte[] out, final int ofs, final int len)
