...
import com.github.gv2011.javapieces.tls.SSLEngineResult;
import com.github.gv2011.javapieces.tls.SSLEngineResult.HandshakeStatus;
import com.github.gv2011.javapieces.tls.SSLEngineResult.Status;
...
final public class SSLEngineImpl extends SSLEngine {
     ...
     */
    private HandshakeStatus getHSStatus(final HandshakeStatus hss) {
     ...
     */
    @Override
    public SSLEngineResult unwrap(final ByteBuffer netData, final ByteBuffer [] appData,
    ...
    private SSLEngineResult readNetRecord(final EngineArgs ea) throws IOException {
         ...
         */
        final int packetLen = inputRecord.bytesInCompletePacket(ea.netData);
     ...
     */
    private HandshakeStatus readRecord(final EngineArgs ea) throws IOException {
        ...
        if (getConnectionState() != cs_ERROR) {
            ...
            try {
                ...
                final byte alertType = (inputRecord.contentType() ==
                    Record.ct_handshake) ?
     ...
     */
    @Override
    public SSLEngineResult wrap(final ByteBuffer [] appData,
            final int offset, final int length, final ByteBuffer netData) throws SSLException {
     ...
     */
    private HandshakeStatus writeRecord(final EngineOutputRecord eor,
            final EngineArgs ea) throws IOException {
     ...
     */
    private boolean checkSequenceNumber(final Authenticator authenticator, final byte type)
            throws IOException {
     ...
     */
    synchronized void fatal(final byte description, String diagnostic,
    ...
    private void recvAlert() throws IOException {
        final byte level = (byte)inputRecord.read();
        final byte description = (byte)inputRecord.read();
    ...
    private void sendAlert(final byte level, final byte description) {
        ...
        final EngineOutputRecord r = new EngineOutputRecord(Record.ct_alert, this);
