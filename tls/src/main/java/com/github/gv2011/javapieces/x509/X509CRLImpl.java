...
public class X509CRLImpl extends X509CRL implements DerEncoder {
        ...
        throws CRLException
    {
        ...
        if (badCerts != null) {
            ...
            for (int i = 0; i < badCerts.length; i++) {
                final X509CRLEntryImpl badCert = (X509CRLEntryImpl)badCerts[i];
                try {
                ...
                revokedMap.put(issuerSerial, badCert);
                revokedList.add(badCert);
    ...
    public void encodeInfo(final OutputStream out) throws CRLException {
        try {
            ...
            if (!revokedList.isEmpty()) {
                for (final X509CRLEntry entry : revokedList) {
    ...
    public String toString() {
        ...
        if (extensions != null) {
            final Collection<Extension> allExts = extensions.getAllExtensions();
    ...
    public String getSigAlgOID() {
        if (sigAlgId == null)
            return null;
        final ObjectIdentifier oid = sigAlgId.getOID();
        return oid.toString();
    }
    ...
    public Set<String> getCriticalExtensionOIDs() {
        ...
        for (final Extension ex : extensions.getAllExtensions()) {
            if (ex.isCritical()) {
    ...
    public byte[] getExtensionValue(final String oid) {
        ...
        try {
            ...
            if (extAlias == null) { // may be unknown
                ...
                for (final Enumeration<Extension> e = extensions.getElements();
                                                 e.hasMoreElements();) {
                    ex = e.nextElement();
    ...
    private void parse(final DerValue val) throws CRLException, IOException {
        ...
        if ((nextByte == DerValue.tag_SequenceOf)
            ...
            X500Principal badCertIssuer = crlIssuer;
            for (int i = 0; i < badCerts.length; i++) {
                final X509CRLEntryImpl entry = new X509CRLEntryImpl(badCerts[i]);
    ...
    private final static class X509IssuerSerial
        ...
        public int compareTo(final X509IssuerSerial another) {
            final int cissuer = issuer.toString()
                    .compareTo(another.issuer.toString());
            if (cissuer != 0) return cissuer;
            return serial.compareTo(another.serial);
        }
    }
}
