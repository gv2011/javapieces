...
implements CertAttrSet<String>, Cloneable {
    ...
    private void calcMinMax() throws IOException {
        ...
        if (excluded != null) {
            for (int i = 0; i < excluded.size(); i++) {
                final GeneralSubtree subtree = excluded.get(i);
        ...
        if (permitted != null) {
            for (int i = 0; i < permitted.size(); i++) {
                final GeneralSubtree subtree = permitted.get(i);
                if (subtree.getMinimum() != 0)
                    hasMin = true;
                if (subtree.getMaximum() != -1)
                    hasMax = true;
    ...
    public boolean verify(final GeneralNameInterface name) throws IOException {
        ...
        if (excluded != null && excluded.size() > 0) {
            ...
            for (int i = 0; i < excluded.size(); i++) {
                final GeneralSubtree gs = excluded.get(i);
                if (gs == null)
                    continue;
        ...
        if (permitted != null && permitted.size() > 0) {
            ...
            for (int i = 0; i < permitted.size(); i++) {
                final GeneralSubtree gs = permitted.get(i);
                if (gs == null)
                    continue;
                final GeneralName gn = gs.getName();
                if (gn == null)
