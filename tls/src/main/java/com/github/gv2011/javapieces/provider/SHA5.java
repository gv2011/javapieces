...
import static com.github.gv2011.javapieces.provider.ByteArrayAccess.b2lBig128;
import static com.github.gv2011.javapieces.provider.ByteArrayAccess.i2bBig4;
import static com.github.gv2011.javapieces.provider.ByteArrayAccess.l2bBig;
...
abstract class SHA5 extends DigestBase {
     ...
     */
    SHA5(final String name, final int digestLength, final long[] initialHashes) {
    ...
    @Override
    final void implDigest(final byte[] out, final int ofs) {
        final long bitsProcessed = bytesProcessed << 3;

        final int index = (int)bytesProcessed & 0x7f;
        final int padLen = (index < 112) ? (112 - index) : (240 - index);
     ...
     * @param z long
     */
    private static long lf_ch(final long x, final long y, final long z) {
        return (x & y) ^ ((~x) & z);
    }
     ...
     * @param z long
     */
    private static long lf_maj(final long x, final long y, final long z) {
        return (x & y) ^ (x & z) ^ (y & z);
    }
     ...
     * @param s int
     */
    private static long lf_R(final long x, final int s) {
        return (x >>> s);
    }
     ...
     * @param s int
     */
    private static long lf_S(final long x, final int s) {
        return (x >>> s) | (x << (64 - s));
    }
     ...
     * @param x long
     */
    private static long lf_sigma0(final long x) {
     ...
     * @param x long
     */
    private static long lf_sigma1(final long x) {
     ...
     * @param x long
     */
    private static long lf_delta0(final long x) {
     ...
     * @param x long
     */
    private static long lf_delta1(final long x) {
     ...
     */
    @Override
    final void implCompress(final byte[] buf, final int ofs) {
        b2lBig128(buf, ofs, W);
        ...
        for (int i = 0; i < ITERATION; i++) {
            final long T1 = h + lf_sigma1(e) + lf_ch(e,f,g) + ROUND_CONSTS[i] + W[i];
            final long T2 = lf_sigma0(a) + lf_maj(a,b,c);
            h = g;
            g = f;
            f = e;
            e = d + T1;
    ...
    public Object clone() throws CloneNotSupportedException {
        final SHA5 copy = (SHA5) super.clone();
