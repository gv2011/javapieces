...
implements java.security.interfaces.DSAKeyPairGenerator {
    ...
    private static void checkStrength(final int sizeP, final int sizeQ) {
    ...
    @Override
    public void initialize(final int modlen, final SecureRandom random) {
        // generate new parameters when no precomputed ones available.
        initialize(modlen, true, random);
        forceNewParameters = false;
    }
     ...
     */
    @Override
    public void initialize(final int modlen, final boolean genParams, final SecureRandom random) {
        int subPrimeLen = -1;
        if (modlen <= 1024) {
            subPrimeLen = 160;
        } else if (modlen == 2048) {
     ...
     */
    @Override
    public void initialize(final DSAParams params, final SecureRandom random) {
        if (params == null) {
            throw new InvalidParameterException("Params must not be null");
        }
        final DSAParameterSpec spec = new DSAParameterSpec
    ...
    private void initialize0(final DSAParameterSpec params, final SecureRandom random) {
        final int sizeP = params.getP().bitLength();
        final int sizeQ = params.getQ().bitLength();
    ...
    public KeyPair generateKeyPair(final BigInteger p, final BigInteger q, final BigInteger g,
                                   final SecureRandom random) {

        final BigInteger x = generateX(random, q);
        final BigInteger y = generateY(x, p, g);
        ...
        try {
            ...
            final DSAPrivateKey priv = new DSAPrivateKey(x, p, q, g);

            final KeyPair pair = new KeyPair(pub, priv);
            return pair;
     ...
     * generateX method.
     */
    private BigInteger generateX(final SecureRandom random, final BigInteger q) {
        BigInteger x = null;
        final byte[] temp = new byte[qlen];
        while (true) {
     ...
     */
    BigInteger generateY(final BigInteger x, final BigInteger p, final BigInteger g) {
