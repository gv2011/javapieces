...
final class RSAClientKeyExchange extends HandshakeMessage {
    ...
    RSAClientKeyExchange(final ProtocolVersion protocolVersion,
            final ProtocolVersion maxVersion,
            final SecureRandom generator, final PublicKey publicKey) throws IOException {
        ...
        try {
            final String s = ((protocolVersion.v >= ProtocolVersion.TLS12.v) ?
            ...
            final Cipher cipher = JsseJce.getCipher(JsseJce.CIPHER_RSA_PKCS1);
     ...
     */
    @SuppressWarnings("deprecation")
    RSAClientKeyExchange(final ProtocolVersion currentVersion,
            final ProtocolVersion maxVersion,
            final SecureRandom generator, final HandshakeInStream input,
            final int messageSize, final PrivateKey privateKey) throws IOException {
    ...
    private static SecretKey generatePreMasterSecret(
            final int clientVersion, final int serverVersion,
            final byte[] encodedSecret, final SecureRandom generator) {
        ...
        try {
            final String s = ((clientVersion >= ProtocolVersion.TLS12.v) ?
                "SunTls12RsaPremasterSecret" : "SunTlsRsaPremasterSecret");
            final KeyGenerator kg = JsseJce.getKeyGenerator(s);
            kg.init(new TlsRsaPremasterSecretParameterSpec(
    ...
    void print(final PrintStream s) throws IOException {
        ...
        final byte[] ba = preMaster.getEncoded();
