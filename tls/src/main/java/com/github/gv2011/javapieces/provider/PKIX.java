...
import com.github.gv2011.javapieces.secutil.PKIXExtendedParameters;
...
public class PKIX {
    ...
    public static boolean isDSAPublicKeyWithoutParams(final PublicKey publicKey) {
        return (publicKey instanceof DSAPublicKey &&
               ((DSAPublicKey)publicKey).getParams() == null);
    }

    static ValidatorParams checkParams(final CertPath cp, final CertPathParameters params)
    ...
    static BuilderParams checkBuilderParams(final CertPathParameters params)
        throws InvalidAlgorithmParameterException
    {
    ...
    static class ValidatorParams {
        ...
        ValidatorParams(final CertPath cp, final PKIXParameters params)
            throws InvalidAlgorithmParameterException
        {
            this(params);
        ...
        ValidatorParams(final PKIXParameters params)
            throws InvalidAlgorithmParameterException
        {
            ...
            // (not supported).
            for (final TrustAnchor anchor : anchors) {
        ...
        void setCertPath(final CertPath cp) {
            certPath = cp;
        }
    ...
    static class BuilderParams extends ValidatorParams {
        ...
        BuilderParams(final PKIXBuilderParameters params)
            throws InvalidAlgorithmParameterException
        {
            super(params);
            checkParams(params);
        }
        private void checkParams(final PKIXBuilderParameters params)
            throws InvalidAlgorithmParameterException
        {
            final CertSelector sel = targetCertConstraints();
            ...
            this.params = params;
            targetSubject = getTargetSubject(
         ...
         */
        private static X500Principal getTargetSubject(final List<CertStore> stores,
                                                      final X509CertSelector sel)
            throws InvalidAlgorithmParameterException
        {
            ...
            final X509Certificate cert = sel.getCertificate();
            if (cert != null) {
    ...
    static class CertStoreTypeException extends CertStoreException {
        ...
        CertStoreTypeException(final String type, final CertStoreException cse) {
    ...
    private static class CertStoreComparator implements Comparator<CertStore> {
        @Override
        public int compare(final CertStore store1, final CertStore store2) {
