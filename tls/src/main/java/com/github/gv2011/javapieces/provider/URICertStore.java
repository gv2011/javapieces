...
import com.github.gv2011.javapieces.secutil.Cache;
import com.github.gv2011.javapieces.secutil.Debug;
import com.github.gv2011.javapieces.x509.AccessDescription;
...
class URICertStore extends CertStoreSpi {
    ...
    private static int initializeTimeout() {
        final Integer tmp = java.security.AccessController.doPrivileged(
     ...
     */
    URICertStore(final CertStoreParameters params)
        ...
        uri = ((URICertStoreParameters) params).uri;
    ...
    static synchronized CertStore getInstance(final URICertStoreParameters params)
     ...
     */
    static CertStore getInstance(final AccessDescription ad) {
        if (!ad.getAccessMethod().equals((Object)
                AccessDescription.Ad_CAISSUERS_Id)) {
            return null;
        }
        final GeneralNameInterface gn = ad.getAccessLocation().getName();
        if (!(gn instanceof URIName)) {
            return null;
        }
        final URI uri = ((URIName) gn).getURI();
        try {
    ...
    public synchronized Collection<X509Certificate> engineGetCertificates
        (final CertSelector selector) throws CertStoreException {
            ...
            try (InputStream in = connection.getInputStream()) {
                ...
                if (oldLastModified != 0) {
                    if (oldLastModified == lastModified) {
                        ...
                        final HttpURLConnection hconn = (HttpURLConnection)connection;
    ...
    private static Collection<X509Certificate> getMatchingCerts
        (final Collection<X509Certificate> certs, final CertSelector selector) {
        // if selector not specified, all certs match
        if (selector == null) {
            return certs;
        }
        final List<X509Certificate> matchedCerts = new ArrayList<>(certs.size());
    ...
    @Override
    @SuppressWarnings("unchecked")
    public synchronized Collection<X509CRL> engineGetCRLs(final CRLSelector selector)
    ...
    private static Collection<X509CRL> getMatchingCRLs
        (final X509CRL crl, final CRLSelector selector) {
    ...
    static class URICertStoreParameters implements CertStoreParameters {
        ...
        @Override public boolean equals(final Object obj) {
            ...
            final URICertStoreParameters params = (URICertStoreParameters) obj;
    ...
    private static class UCS extends CertStore {
        protected UCS(final CertStoreSpi spi, final Provider p, final String type,
