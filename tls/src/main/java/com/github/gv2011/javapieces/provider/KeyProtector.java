...
import com.github.gv2011.javapieces.pkcs.PKCS8Key;
...
final class KeyProtector {
     ...
     */
    public KeyProtector(final char[] password)
    ...
    public byte[] protect(final Key key) throws KeyStoreException
    {
        ...
        final byte[] plainKey = key.getEncoded();
        if (plainKey == null) {
            throw new KeyStoreException(
        ...
        final byte[] xorKey = new byte[plainKey.length];
        ...
        final byte[] tmpKey = new byte[plainKey.length];
        for (i = 0; i < tmpKey.length; i++) {
            tmpKey[i] = (byte)(plainKey[i] ^ xorKey[i]);
        }

        // Store salt and "tmpKey" in "encrKey"
        final byte[] encrKey = new byte[salt.length + tmpKey.length + DIGEST_LEN];
     ...
     */
    public Key recover(final EncryptedPrivateKeyInfo encrInfo)
        throws UnrecoverableKeyException
    {
        ...
        final byte[] plainKey = new byte[encrKey.length];
