...
class AlgIdDSA extends AlgorithmId implements DSAParams
{
    ...
    AlgIdDSA (final DerValue val) throws IOException
        { super(val.getOID()); }

    /**
     * Construct an AlgIdDSA from an X.509 encoded byte array.
     */
    public AlgIdDSA (final byte[] encodedAlg) throws IOException
     ...
     */
    public AlgIdDSA (final byte p [], final byte q [], final byte g [])
    throws IOException
    {
     ...
     * @param g the DSS/DSA parameter "G"
     */
    public AlgIdDSA (final BigInteger p, final BigInteger q, final BigInteger g)
    {
        super (DSA_oid);
