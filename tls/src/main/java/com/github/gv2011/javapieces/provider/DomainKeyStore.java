...
abstract class DomainKeyStore extends KeyStoreSpi {
    ...
    {
        final AbstractMap.SimpleEntry<String, Collection<KeyStore>> pair =
            getKeystoresForReading(alias);
        Key key = null;

        try {
            final String entryAlias = pair.getKey();
            for (final KeyStore keystore : pair.getValue()) {
    ...
    {
        final AbstractMap.SimpleEntry<String,
        ...
        final String entryAlias = pair.getKey();
        final Map.Entry<String, KeyStore> keystore = pair.getValue();
    ...
    private AbstractMap.SimpleEntry<String, Collection<KeyStore>>
        getKeystoresForReading(final String alias) {

        final String[] splits = alias.split(entryNameSeparatorRegEx, 2);
        if (splits.length == 2) { // prefixed alias
            final KeyStore keystore = keystores.get(splits[0]);
            if (keystore != null) {
    ...
    AbstractMap.SimpleEntry<String, AbstractMap.SimpleEntry<String, KeyStore>>
        getKeystoreForWriting(final String alias) {

        final String[] splits = alias.split(entryNameSeparator, 2);
        if (splits.length == 2) { // prefixed alias
            final KeyStore keystore = keystores.get(splits[0]);
            if (keystore != null) {
                return new AbstractMap.SimpleEntry<>(splits[1],
        ...
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        if (param instanceof DomainLoadStoreParameter) {
            final DomainLoadStoreParameter domainParameter =
                (DomainLoadStoreParameter) param;
            final List<KeyStoreBuilderComponents> builders = getBuilders(
                domainParameter.getConfiguration(),
                    domainParameter.getProtectionParams());

            for (final KeyStoreBuilderComponents builder : builders) {

                try {

                    final KeyStore.ProtectionParameter pp = builder.protection;
                    ...
                    // Store the keystores
                    final KeyStore keystore = keystores.get(builder.name);
        ...
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        ...
        try {
            ...
            final String keystoreName = DEFAULT_STREAM_PREFIX + streamCounter++;
     ...
     */
    private List<KeyStoreBuilderComponents> getBuilders(final URI configuration,
        final Map<String, KeyStore.ProtectionParameter> passwords)
            throws IOException {

        final PolicyParser parser = new PolicyParser(true); // expand properties
        Collection<PolicyParser.DomainEntry> domains = null;
        final List<KeyStoreBuilderComponents> builders = new ArrayList<>();
        final String uriDomain = configuration.getFragment();
        ...
        for (final PolicyParser.DomainEntry domain : domains) {
            final Map<String, String> domainProperties = domain.getProperties();
            ...
            if (domainProperties.containsKey(ENTRY_NAME_SEPARATOR)) {
                ...
                for (int i = 0; i < entryNameSeparator.length(); i++) {
                    ch = entryNameSeparator.charAt(i);
                    if (REGEX_META.indexOf(ch) != -1) {
                        s.append('\\');
                    }
                    s.append(ch);
                }
                entryNameSeparatorRegEx = s.toString();
            }

            final Collection<PolicyParser.KeyStoreEntry> keystores =
                domain.getEntries();
            for (final PolicyParser.KeyStoreEntry keystore : keystores) {
                final String keystoreName = keystore.getName();
                ...
                if (properties.containsKey(KEYSTORE_PROVIDER_NAME)) {
                    final String keystoreProviderName =
                ...
                if (properties.containsKey(KEYSTORE_URI)) {
                    final String uri = properties.get(KEYSTORE_URI);
                ...
                if (passwords.containsKey(keystoreName)) {
                    ...
                    final String env = properties.get(KEYSTORE_PASSWORD_ENV);
                    final String pwd = System.getenv(env);
                    if (pwd != null) {
                        keystoreProtection =
...
class KeyStoreBuilderComponents {
    ...
    KeyStoreBuilderComponents(final String name, final String type, final Provider provider,
        final File file, final KeyStore.ProtectionParameter protection) {
        this.name = name;
        this.type = type;
