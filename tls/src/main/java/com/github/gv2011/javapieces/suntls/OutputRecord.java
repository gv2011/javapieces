...
class OutputRecord extends ByteArrayOutputStream implements Record {
    ...
    OutputRecord(final byte type, final int size) {
        ...
        helloVersion = ProtocolVersion.DEFAULT_HELLO;
        firstMessage = true;
     ...
     */
    void doHashes() {
        final int len = count - lastHashed;
     ...
     */
    void encrypt(final Authenticator authenticator, final CipherBox box)
            throws IOException {
        ...
        if (authenticator instanceof MAC) {
            ...
            if (signer.MAClen() != 0) {
                final byte[] hash = signer.compute(contentType, buf,
        ...
        if (!box.isNullCipher()) {
            ...
            if ((protocolVersion.v >= ProtocolVersion.TLS11.v) &&
                ...
                final byte[] nonce = box.createExplicitNonce(authenticator,
                                    contentType, count - headerPlusMaxIVSize);
                final int offset = headerPlusMaxIVSize - nonce.length;
    ...
    final int availableDataBytes() {
        final int dataSize = count - headerPlusMaxIVSize;
     ...
     */
    void write(final OutputStream s, final boolean holdRecord,
            final ByteArrayOutputStream heldRecordBuffer) throws IOException {
        ...
        final int length = count - headerOffset - headerSize;
        ...
        if (holdRecord) {
            ...
            if (heldRecordBuffer != null && heldRecordBuffer.size() > 0) {
                ...
                final int newCount = count + heldLen - headerOffset;
    ...
    private void V3toV2ClientHello(final byte v3Msg []) throws SSLException {
        final int v3SessionIdLenOffset = 2 + 32; // version + nonce
        final int v3SessionIdLen = v3Msg[v3SessionIdLenOffset];
        final int v3CipherSpecLenOffset = v3SessionIdLenOffset + 1 + v3SessionIdLen;
        final int v3CipherSpecLen = ((v3Msg[v3CipherSpecLenOffset] & 0xff) << 8) +
          (v3Msg[v3CipherSpecLenOffset + 1] & 0xff);
        final int cipherSpecs = v3CipherSpecLen / 2; // 2 bytes each in V3
