...
public final class Signal {
    ...
    public boolean equals(final Object other) {
        ...
        final Signal other1 = (Signal)other;
     ...
     */
    public static synchronized Signal.Handler handle(final Signal sig,
                                                    final Signal.Handler handler)
        throws IllegalArgumentException {
        Objects.requireNonNull(sig, "sig");
        Objects.requireNonNull(handler, "handler");
        final long newH = (handler instanceof NativeHandler) ?
                      ((NativeHandler)handler).getHandler() : 2;
        final long oldH = handle0(sig.number, newH);
        if (oldH == -1) {
        ...
        synchronized (handlers) {
            final Signal.Handler oldHandler = handlers.get(sig);
            handlers.remove(sig);
            if (newH == 2) {
     ...
     */
    public static void raise(final Signal sig) throws IllegalArgumentException {
        Objects.requireNonNull(sig, "sig");
    ...
    private static void dispatch(final int number) {
        ...
        final Runnable runnable = () -> handler.handle(sig);
        if (handler != null) {
    ...
    static final class NativeHandler implements Signal.Handler {
        ...
        NativeHandler(final long handler) {
