...
public final class ECParameters extends AlgorithmParametersSpi {
    ...
    public static AlgorithmParameters getAlgorithmParameters(final ECParameterSpec spec)
    ...
    protected void engineInit(final AlgorithmParameterSpec paramSpec)
        ...
        if (paramSpec instanceof ECParameterSpec) {
            ...
            final String name = ((ECGenParameterSpec)paramSpec).getName();
            namedCurve = CurveDB.lookup(name);
        } else if (paramSpec instanceof ECKeySizeParameterSpec) {
            final int keySize = ((ECKeySizeParameterSpec)paramSpec).getKeySize();
    ...
    protected void engineInit(final byte[] params) throws IOException {
        ...
        if (encodedParams.tag == DerValue.tag_ObjectId) {
            final ObjectIdentifier oid = encodedParams.getOID();
            final NamedCurve spec = CurveDB.lookup(oid.toString());
            if (spec == null) {
...
*/
    ...
    protected <T extends AlgorithmParameterSpec> T
            engineGetParameterSpec(final Class<T> spec)
            throws InvalidParameterSpecException {
        ...
        if (spec.isAssignableFrom(ECGenParameterSpec.class)) {
            ...
            final String name = namedCurve.getObjectId();
            return spec.cast(new ECGenParameterSpec(name));
        }

        if (spec.isAssignableFrom(ECKeySizeParameterSpec.class)) {
            final int keySize = namedCurve.getCurve().getField().getFieldSize();
